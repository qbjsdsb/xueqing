...
-- A monotonic claim sequence records the serialized command-claim order and
-- keeps committed close ordering deterministic across equal transaction times.
...
comment on column public.operation_receipts.receipt_claim_sequence is
  'Monotonic operation-claim sequence used as the primary lifecycle order; committed_at and id remain diagnostic tie-breakers.';
...
begin
  ...
  if case_status = 'closed' then
    ...
    order by receipt.receipt_claim_sequence desc,
             receipt.committed_at desc,
             receipt.id desc
    limit 1
    for update of event;
