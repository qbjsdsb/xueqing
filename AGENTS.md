...
本文件是 Codex 与后续开发者的**硬约束索引**。详细设计以 `docs/` 中对应专题文档为准；不要在代码里静默推翻已接受 ADR。
...
V1 暂不做：收费排课 CRM、大型题库、成绩预测/学情综合分、家长/学生独立 App、AI 自动正式诊断、Google Docs 式协同、复杂离线多主同步、庞大知识图谱。
...
- Auth 登录身份与机构授权分离。
- 管理员创建的是 `organization_invitation(email)` 预授权，不是 Supabase Invite Link。
- 仅 Auth User、没有 active membership 时不能读取机构业务数据。
- pending invitation 也没有业务数据权限。
- verified email 接受 invitation 后，受控事务创建 active membership + roles。
- membership disabled 后，即使旧 Token 尚有效，RLS 仍必须拒绝机构数据。
- 权限事实源是 membership / roles / assignments，不是 `user_metadata`。
- OTP Production 需验证 rate limits / CAPTCHA 与可靠 Custom SMTP/等价邮件服务。
...
### Flutter
- Windows + Android 优先。
- View / ViewModel / Repository / Service 分层，feature-first。
...
### Secret
Flutter/GitHub 绝不包含：Secret Key、service_role、数据库密码、SMTP/AI/第三方私钥。

## 6. 环境与 Migration

必须区分：
- **Local Development**：Supabase CLI、虚构 seed、schema/RLS/tests；
- **Remote Development**：虚构数据，用于 OTP 邮件、Storage、Edge Functions、Windows/Android 公网联调；
- **Production**：真实机构数据。
...
详细流程：`docs/DEVELOPMENT_WORKFLOW.md`。
...
受控命令至少考虑：权限、当前状态、expected_version、多表原子性、operation_id/幂等。
...
- DB/RLS tests 通过；
- 未登录、无 membership、pending invitation、disabled membership、跨机构等负面场景按预期拒绝。
...
PR 具体检查项见 `.github/pull_request_template.md`。