...
- `LearningRepository` 暴露 Case 确认、教学动作、验证、Action 改期和 Action 完成事务命令；UI 只追加教学事实或通过受控命令推进状态，不直接编辑历史；
- Quick Capture 和所有后续命令都使用一次生成、重试复用的 `operation_id`，服务端成功前不关闭表单；完成 Action 时必须同时填写下一行动，服务端原子保留当前 Case 的后续责任链；
