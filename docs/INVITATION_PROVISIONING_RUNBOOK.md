...
当前最终 migration 链把成员账号生命周期写权限收紧为 `org_owner` only。管理员可以查看成员/邀请状态、维护显示姓名，以及管理学生、学科、教学范围和任课关系，但不能创建/处理邀请、停用/恢复成员或重发成员凭据。

负责人在工作台进入「机构管理 → 邀请成员」：

- 新邮箱没有 Auth 账号：系统创建 Auth User，建立 `onboarding` membership，临时密码只在成功响应中显示一次。负责人应通过可信渠道交付邮箱、临时密码和有效期。
...
- 负责人不能直接把 `onboarding` 成员改成 `active`；受邀人必须先完成首次接管。
- `teacher`、`org_admin`、`org_owner` 邀请都必须由负责人发起；不得让管理员借较早 migration 的历史行为绕过当前 owner-only helper。
...
临时密码丢失或响应没有交付成功时，不能找回旧明文。负责人在成员列表点击「重新发放临时密码」，旧密码立即失效，成员仍保持 `onboarding`。

## 最小验收清单

- [ ] 新邮箱能被负责人开通，临时密码只显示一次，数据库和日志中没有明文密码。
- [ ] org_admin 尝试创建/审批/撤销/重发邀请、停用/恢复成员或重发凭据时，被 owner-only helper 以 `organization_owner_required` 拒绝。
...
- [ ] 响应丢失后，系统先自动核对已提交事务；若仍不确定，负责人可以继续开通或重新发放新密码，旧密码不能登录，新密码可以接管。
...
- 负责人没有收到临时密码：不要查数据库或日志找密码，直接重新发放。
- 接管失败：成员应保持 `onboarding`；先检查有效期、密码更新时间和重新登录，再由负责人尝试重新发放。
