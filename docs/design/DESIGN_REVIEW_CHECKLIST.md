...
CI 证据：GitHub Actions `Flutter checks` run #41（Flutter 3.47.1，Ubuntu）已通过 `flutter pub get`、lockfile consistency、Dart format、`flutter analyze` 与全部 widget tests；详见 [run #41](https://github.com/qbjsdsb/xueqing/actions/runs/33615144817)。本地执行环境没有 Flutter/Dart，因此不把本地缺失工具链误写成通过。
