...
2. “现在最重要的事”：最多 3 个未关闭问题，每项指向 Case 或 action，并有明确下一步。
3. 若还有其他问题，在同一 section 提供 `查看全部 N 个`；展开后显示全部当前与已关闭问题，并允许 `只看重点` 返回。
4. 未进入当前重点的 `pending verification` 问题可以另做最多 2 项的有限预览；查看全部时不再重复显示该区。
5. 最近关键事实：最近 Evidence / Lesson / Intervention 的少量摘要。
6. 必要学科上下文：只有确实帮助理解当前问题时才显示为一行定位文字，不再占用独立 notice/card。
7. 更早历史：折叠或 timeline，按需展开。

“三件事”是首屏认知上限，不是新的业务字段或评分。若只有一件事，就只展示一件；若没有当前需要跟进的问题但有关闭历史，显示清楚的空状态并保留“查看全部”入口；若完全没有 Case，显示“还没有记录的问题”和“记录问题”入口。关闭 Case 不计入学生列表的当前问题数量。
