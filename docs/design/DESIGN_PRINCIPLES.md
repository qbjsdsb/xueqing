...
学生详情首屏必须让第一次接手的教师在不翻完整历史的情况下回答“现在最重要的三件事”。默认只展示最多 3 个未关闭的当前重点，并把完整问题清单放到同一 section 的按需展开入口里；关闭历史只在展开全部问题时出现，不当作当前工作。未进入当前重点的待验证问题可以保留一个有限预览，但不能再铺一份与主列表重复的完整 Case 区块。学科上下文只有在确实帮助理解当前问题时才用一行定位文字出现，最近关键事实和更早历史继续按“当前优先、历史按需”组织。
