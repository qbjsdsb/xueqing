...
- 图片附件 PR #51 已重基到该主线，并补上上传中会话切换保护、失败重试复用同一 attachment ID、以及附件注册 RPC 的 public/private 安全边界；集成 CI 已通过（Flutter 589、Supabase 341、platform smoke 240）。Android / Windows 归档只证明包已构建，真实设备 gate 仍未完成，未通过前不视为主线能力。
...
- [x] PR #51 图片附件：集成 head `c0e9a58` 的 Flutter / Supabase / platform CI 已通过；[ ] 真实 Android / Windows、无代理网络、签名预览和远端开发迁移 gate 全部通过后再合入主线。
