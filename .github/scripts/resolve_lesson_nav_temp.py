...
Path("docs/product/LESSON_NAVIGATION_DECISION.md").write_text(
...
replace_one(
    "docs/product/ROLE_ACCESS_CONTRACT.md",
...
replace_one(
    ...
    "responsibility lesson test contract",
)

replace_one(
    "docs/product/DUAL_WORKSPACE_UX.md",
...
checks = {
    "docs/product/ROLE_ACCESS_CONTRACT.md": ["今日 / 学生 / 学情", "LESSON_NAVIGATION_DECISION.md"],
    "docs/RESPONSIBILITY_MODEL.md": ["普通任课老师：今日 / 学生 / 学情", "当前 v0.3.x 不展示该入口"],
    "docs/product/DUAL_WORKSPACE_UX.md": ["v0.3.x 不恢复“课程”一级入口"],
    "docs/design/INFORMATION_ARCHITECTURE.md": ["运行时说明：v0.3.x 当前生产 V2", "课程（未来）"],
    "docs/design/SCREEN_SPECS.md": ["未来目标态路径"],
    "README.md": ["当前生产 V2 只开放 3 个真实核心入口", "LESSON_NAVIGATION_DECISION.md"],
}
for path, needles in checks.items():
    text = Path(path).read_text()
    for needle in needles:
        if needle not in text:
            raise SystemExit(f"{path}: missing expected contract text: {needle}")

if "今日 / 学生 / 课程 / 学情" in Path("docs/product/ROLE_ACCESS_CONTRACT.md").read_text():
    raise SystemExit("ROLE_ACCESS_CONTRACT still claims stale four-tab current runtime")
if "普通任课老师：今日 / 学生 / 课程 / 学情" in Path("docs/RESPONSIBILITY_MODEL.md").read_text():
    raise SystemExit("RESPONSIBILITY_MODEL still claims stale four-tab current runtime")
