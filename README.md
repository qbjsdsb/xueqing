...
课堂中允许先用 `new` 草稿快速捕捉；进入 `confirmed` 后再要求正式分类和下一步。复发时执行 reopen 命令重新激活原案例，而不是创建第七个状态或不断复制同一问题。

### 下一步行动驱动
从 `confirmed` 开始的主动跟进案例通常存在一个当前主行动，例如次课验证、相似题训练或重新讲解；如果暂不安排，应有明确暂停理由。“今日”主要围绕这些行动组织，而不是依赖完整排课系统。
...
- 学生不会因为老师/学科/年级重复建档；
- 一个案例从证据、干预到验证完整可追溯；
- `confirmed` 及后续主动跟进案例有主行动或明确暂停理由；
- 常规课后记录中位时间目标 ≤ 60 秒；
- 短暂网络失败不丢正在编辑内容；
- 教师交接不丢历史；
- 跨机构访问在数据库层拒绝；
...
3. 重要学情结论尽量有证据。
4. `new` 允许快速捕捉；从 `confirmed` 开始必须有明确下一步行动或暂停理由。
5. 数据归机构，人员变化不能破坏历史。
6. 权限必须在数据库/服务端执行。
